<?php
  ...
  // We also need to eventually.  Use the page owners settings for everybody - get_pconfig(page_owner()) or whatever that would look like.
    ...
    $site_colour = get_config("blogbasic", "colour" );
    $shadows = get_config("blogbasic", "shadow" );
    $navcolour = get_config("blogbasic", "navcolour" );
    $displaystyle = get_config("blogbasic", "displaystyle" );
    ...
    if ($uid) {
        ...
        $colour = get_pconfig($uid, "blogbasic", "colour");
        $shadows = get_pconfig($uid, "blogbasic", "shadow");
        $navcolour = get_pconfig($uid, "blogbasic", "navcolour");
        $displaystyle = get_pconfig($uid, "blogbasic", "displaystyle");
        $linkcolour = get_pconfig($uid, "blogbasic", "linkcolour");
        $shiny = get_pconfig($uid, "blogbasic", "shiny");
		$x = get_pconfig($uid,'blogbasic','radius');
		if($x !== false)
			$radius = $x;

        if (! feature_enabled($uid,'expert')) {$colour_scheme = get_pconfig($uid, "blogbasic", "colour_scheme");}
    }
