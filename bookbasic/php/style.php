<?php
  ...
  // We also need to eventually.  Use the page owners settings for everybody - get_pconfig(page_owner()) or whatever that would look like.
	...
	if($uid)
	    load_pconfig($uid,'bookbasic');

    $line_height = false;
    $bookbasic_font_size = false;
    $resolution = false;
    $navcolour = false;
    $nav_bg_1 = "f88";
    $nav_bg_2 = "b00";
	$nav_bg_3 = "f00";
	$nav_bg_4 = "b00";
    $site_line_height = get_config("bookbasic","line_height");
    $site_bookbasic_font_size = get_config("bookbasic", "font_size" );
    $navcolour = get_config("bookbasic", "navcolour" );
    
    if ($uid) {
        $line_height = get_pconfig($uid, "bookbasic","line_height");
        $bookbasic_font_size = get_pconfig($uid, "bookbasic", "font_size");
        $navcolour = get_pconfig($uid, "bookbasic", "navcolour");

        if (! feature_enabled($uid,'expert')) {$colour_scheme = get_pconfig($uid, "bookbasic", "colour_scheme");}
    }
    ...
    if(! feature_enabled($uid,'expert')) {
	    if ($colour_scheme === 'bookbasic'){$navcolour = 'red';}
		$shadows = false;
}
...
// eg, if ($bookbasic_font_size === false) {$bookbasic_font_size = get_pconfig($uid, "global", "font_size");  If it's not set, we'll just use the CSS with no changes.
// Then all you need to do is add a "Global Settings" tab to settings/display, and make an equivalent of theme_settings.tpl and config.php to be loaded there.  Easy.
    ...
    if ($bookbasic_font_size === false) {$bookbasic_font_size = $site_bookbasic_font_size;}
    if ($bookbasic_font_size === false) {$bookbasic_font_size = "12";}
...
// Enforce sane limits for expert mode - otherwise we'll end up with "experts" who think font size is a percentage.
	...
	if(($bookbasic_font_size >= 8.0) && ($bookbasic_font_size <= 20.0)) {
		echo ".wall-item-content { font-size: ${bookbasic_font_size}px;}\r\n";
	}
