<?php
  ...
  // We also need to eventually.  Use the page owners settings for everybody - get_pconfig(page_owner()) or whatever that would look like.
	...
	if($uid)
	    load_pconfig($uid,'redblog');

    $line_height = false;
    $redblog_font_size = false;
    $resolution = false;
    $colour = false;
    $shadows = false;
    $navcolour = false;
    ...
    $shiny = false;
	$radius = 5;
    $site_line_height = get_config("redblog","line_height");
    $site_redblog_font_size = get_config("redblog", "font_size" );
    $site_colour = get_config("redblog", "colour" );
    $shadows = get_config("redblog", "shadow" );
    $navcolour = get_config("redblog", "navcolour" );
    $displaystyle = get_config("redblog", "displaystyle" );
    $linkcolour = get_config("redblog", "linkcolour" );
    $shiny = get_config("redblog", "shiny" );

	$x = get_config('redblog','radius');
	if($x !== false)
		$radius = $x;
    
    if ($uid) {
        $line_height = get_pconfig($uid, "redblog","line_height");
        $redblog_font_size = get_pconfig($uid, "redblog", "font_size");
        $colour = get_pconfig($uid, "redblog", "colour");
        $shadows = get_pconfig($uid, "redblog", "shadow");
        $navcolour = get_pconfig($uid, "redblog", "navcolour");
        $displaystyle = get_pconfig($uid, "redblog", "displaystyle");
        $linkcolour = get_pconfig($uid, "redblog", "linkcolour");
        $shiny = get_pconfig($uid, "redblog", "shiny");
		$x = get_pconfig($uid,'redblog','radius');
		if($x !== false)
			$radius = $x;

        if (! feature_enabled($uid,'expert')) {$colour_scheme = get_pconfig($uid, "redblog", "colour_scheme");}
    }
    ...
    if(! feature_enabled($uid,'expert')) {
	    ...
	    if ($colour_scheme === 'redblog'){$navcolour = 'red';}
		$shadows = false;
		$radius = 5;
}
...
// eg, if ($redblog_font_size === false) {$redblog_font_size = get_pconfig($uid, "global", "font_size");  If it's not set, we'll just use the CSS with no changes.
// Then all you need to do is add a "Global Settings" tab to settings/display, and make an equivalent of theme_settings.tpl and config.php to be loaded there.  Easy.
    ...
    if ($redblog_font_size === false) {$redblog_font_size = $site_redblog_font_size;}
    if ($redblog_font_size === false) {$redblog_font_size = "12";}
...
// Enforce sane limits for expert mode - otherwise we'll end up with "experts" who think font size is a percentage.
	...
	if(($redblog_font_size >= 8.0) && ($redblog_font_size <= 20.0)) {
		echo ".wall-item-content { font-size: ${redblog_font_size}px;}\r\n";
	}
