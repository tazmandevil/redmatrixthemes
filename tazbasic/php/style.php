<?php
  ...
  // We also need to eventually.  Use the page owners settings for everybody - get_pconfig(page_owner()) or whatever that would look like.
	...
	if($uid)
	    load_pconfig($uid,'tazbasic');

    $line_height = false;
    $tazbasic_font_size = false;
    $resolution = false;
    $colour = false;
    $shadows = false;
    $navcolour = false;
    ...
    $shiny = false;
	$radius = 5;
    $site_line_height = get_config("tazbasic","line_height");
    $site_tazbasic_font_size = get_config("tazbasic", "font_size" );
    $site_colour = get_config("tazbasic", "colour" );
    $shadows = get_config("tazbasic", "shadow" );
    $navcolour = get_config("tazbasic", "navcolour" );
    $displaystyle = get_config("tazbasic", "displaystyle" );
    $linkcolour = get_config("tazbasic", "linkcolour" );
    $shiny = get_config("tazbasic", "shiny" );

	$x = get_config('tazbasic','radius');
	if($x !== false)
		$radius = $x;
    
    if ($uid) {
        $line_height = get_pconfig($uid, "tazbasic","line_height");
        $tazbasic_font_size = get_pconfig($uid, "tazbasic", "font_size");
        $colour = get_pconfig($uid, "tazbasic", "colour");
        $shadows = get_pconfig($uid, "tazbasic", "shadow");
        $navcolour = get_pconfig($uid, "tazbasic", "navcolour");
        $displaystyle = get_pconfig($uid, "tazbasic", "displaystyle");
        $linkcolour = get_pconfig($uid, "tazbasic", "linkcolour");
        $shiny = get_pconfig($uid, "tazbasic", "shiny");
		$x = get_pconfig($uid,'tazbasic','radius');
		if($x !== false)
			$radius = $x;

        if (! feature_enabled($uid,'expert')) {$colour_scheme = get_pconfig($uid, "tazbasic", "colour_scheme");}
    }
    ...
    if(! feature_enabled($uid,'expert')) {
	    ...
	    if ($colour_scheme === 'tazbasic'){$navcolour = 'red';}
		$shadows = false;
		$radius = 5;
}
...
// eg, if ($tazbasic_font_size === false) {$tazbasic_font_size = get_pconfig($uid, "global", "font_size");  If it's not set, we'll just use the CSS with no changes.
// Then all you need to do is add a "Global Settings" tab to settings/display, and make an equivalent of theme_settings.tpl and config.php to be loaded there.  Easy.
    ...
    if ($tazbasic_font_size === false) {$tazbasic_font_size = $site_tazbasic_font_size;}
    if ($tazbasic_font_size === false) {$tazbasic_font_size = "12";}
...
// Enforce sane limits for expert mode - otherwise we'll end up with "experts" who think font size is a percentage.
	...
	if(($tazbasic_font_size >= 8.0) && ($tazbasic_font_size <= 20.0)) {
		echo ".wall-item-content { font-size: ${tazbasic_font_size}px;}\r\n";
	}
