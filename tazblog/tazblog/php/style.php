<?php
  ...
  // We also need to eventually.  Use the page owners settings for everybody - get_pconfig(page_owner()) or whatever that would look like.
	...
	if($uid)
	    load_pconfig($uid,'tazblog');

    $line_height = false;
    $tazblog_font_size = false;
    $resolution = false;
    $colour = false;
    $shadows = false;
    $navcolour = false;
    ...
    $shiny = false;
	$radius = 5;
    $site_line_height = get_config("tazblog","line_height");
    $site_tazblog_font_size = get_config("tazblog", "font_size" );
    $site_colour = get_config("tazblog", "colour" );
    $shadows = get_config("tazblog", "shadow" );
    $navcolour = get_config("tazblog", "navcolour" );
    $displaystyle = get_config("tazblog", "displaystyle" );
    $linkcolour = get_config("tazblog", "linkcolour" );
    $shiny = get_config("tazblog", "shiny" );

	$x = get_config('tazblog','radius');
	if($x !== false)
		$radius = $x;
    
    if ($uid) {
        $line_height = get_pconfig($uid, "tazblog","line_height");
        $tazblog_font_size = get_pconfig($uid, "tazblog", "font_size");
        $colour = get_pconfig($uid, "tazblog", "colour");
        $shadows = get_pconfig($uid, "tazblog", "shadow");
        $navcolour = get_pconfig($uid, "tazblog", "navcolour");
        $displaystyle = get_pconfig($uid, "tazblog", "displaystyle");
        $linkcolour = get_pconfig($uid, "tazblog", "linkcolour");
        $shiny = get_pconfig($uid, "tazblog", "shiny");
		$x = get_pconfig($uid,'tazblog','radius');
		if($x !== false)
			$radius = $x;

        if (! feature_enabled($uid,'expert')) {$colour_scheme = get_pconfig($uid, "tazblog", "colour_scheme");}
    }
    ...
    if(! feature_enabled($uid,'expert')) {
	    ...
	    if ($colour_scheme === 'tazblog'){$navcolour = 'red';}
		$shadows = false;
		$radius = 5;
}
...
// eg, if ($tazblog_font_size === false) {$tazblog_font_size = get_pconfig($uid, "global", "font_size");  If it's not set, we'll just use the CSS with no changes.
// Then all you need to do is add a "Global Settings" tab to settings/display, and make an equivalent of theme_settings.tpl and config.php to be loaded there.  Easy.
    ...
    if ($tazblog_font_size === false) {$tazblog_font_size = $site_tazblog_font_size;}
    if ($tazblog_font_size === false) {$tazblog_font_size = "12";}
...
// Enforce sane limits for expert mode - otherwise we'll end up with "experts" who think font size is a percentage.
	...
	if(($tazblog_font_size >= 8.0) && ($tazblog_font_size <= 20.0)) {
		echo ".wall-item-content { font-size: ${tazblog_font_size}px;}\r\n";
	}
